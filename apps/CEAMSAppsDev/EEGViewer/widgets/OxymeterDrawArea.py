...
class OxymeterDrawArea(QWidget):
    def __init__(self, parent):
        ...
        self._arrow_pen = QPen(Qt.black)
        self._arrow_pen.setWidth(2)
    ...
    def pub_sub_manager(self, value):
        self._pub_sub_manager = value
    ...
    @time_cursor.setter
    def time_cursor(self, time_cursor):
    ...
    def paintEvent(self, event):
        ...
        #self._paint_time_cursor(painter)
    ...
    def mouseDoubleClickEvent(self, event):
        if len(self._signal_models) == 0:
            return
        seconds = self._px_to_seconds(event.pos().x())
        self._pub_sub_manager.publish(self, "time_cursor_changed", seconds)
    ...
    # PRIVATE FUNCTIONS
    def _paint_time_cursor(self, painter: QPainter):
        if len(self._signal_models) == 0:
            return
        ...
        x = self._seconds_to_pixel(self._time_cursor)
        painter.drawLine(x, self._top_padding, x, self.height() - self._bottom_padding)

        painter.setPen(QPen(Qt.black, 2, Qt.SolidLine))
        # Draw arrow pointing down on top of the line inside the toppadding section
        painter.drawLine(x, 0, x, self._top_padding-2)
        painter.drawLine(x-3, self._top_padding - 7, x, self._top_padding-2)
        painter.drawLine(x+3, self._top_padding - 7, x, self._top_padding-2)
        ...
        # Draw the creation_date
        try:
            current_time = self._time_cursor + self._file_info["creation_date"]
            current_datetime = datetime.datetime.fromtimestamp(current_time)
            current_time_text = current_datetime.strftime("%H:%M:%S")
            painter.drawText(x+6, self._top_padding - 5, current_time_text)
        except:
            pass
    ...
    def _paint_signals(self, painter):
        ...
        for signal_model in self._signal_models:
            ...
            #y = self._sample_to_pixel(signal_model.samples[0])
            
            #path.moveTo(self._left_padding,y)
    ...
    def _paint_exclusion_events(self, painter: QPainter):
        if len(self._signal_models) == 0:
            return
        # Remove unsaved selections
            # Remove only the new selections and not the saved ones
        # Remove all selections
            # Remove all selections including the saved ones
    ...
    def _paint_x_axis_labels(self, painter, labels):
        ...
        painter.setBrush(Qt.NoBrush)
        pointy_y = self.height() - self._bottom_padding + 5
        bottom_y = self.height() - 5
        painter.drawLine(self._left_padding, bottom_y, self._left_padding,pointy_y)
        painter.drawLine(self._left_padding-3,pointy_y + 5, self._left_padding,pointy_y)
        painter.drawLine(self._left_padding+3,pointy_y + 5, self._left_padding,pointy_y)
        ...
        # Draw the creation_date
        try:
            creation_date = datetime.datetime.fromtimestamp(self._file_info["creation_date"])
            creation_date_text = creation_date.strftime("%H:%M:%S")
            x = self._left_padding+5
            painter.drawText(x, bottom_y, creation_date_text)
        except:
            pass
