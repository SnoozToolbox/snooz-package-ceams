...
class PSACompilationFOOOF(SciNode):
    ...
    def __init__(self, **kwargs):
        """ Initialize module PSACompilationFOOOF """
        super().__init__(**kwargs)
        if DEBUG: print('PSACompilationFOOOF.__init__')
        ...
        # Dict to accumulate the act per stage, hour and cycle for rhythmic and arhythmic signals.
        self.PSD_rhythmic_act_param = {}
        self.PSD_arhythmic_act_param = {}
    ...
    def compute(self, subject_info, PSD, sleep_stages, events, mini_bandwidth, first_freq, last_freq, dist_total, \
        ...
        if not isinstance(PSD,list):
            raise NodeInputException(self.identifier, "PSA", \
                f"PSACompilationFOOOF input of wrong type. Expected: <class 'list'> received: {type(PSD)}")   

        if not isinstance(sleep_stages,pd.DataFrame):
            raise NodeInputException(self.identifier, "sleep_stages", \
                f"PSACompilationFOOOF input of wrong type. Expected: <class 'pd.DataFrame'> received: {type(sleep_stages)}")

        if type(parameters_cycle) != dict:
            raise NodeInputException(self.identifier, "parameters_cycle", \
                f"PSACompilationFOOOF input of wrong type. Expected: <class 'dict'> received: {type(parameters_cycle)}")
        ...
        if (not isinstance(cycle_labelled, pd.DataFrame)) or len(cycle_labelled)==0:
            raise NodeInputException(self.identifier, "cycle_labelled", \
                f"PSACompilationFOOOF input of wrong type or empty. Expected: <class 'pd.DataFrame'> received: {type(cycle_labelled)}")            
        try:
            mini_bandwidth = float(mini_bandwidth)
        except ValueError:
            raise NodeInputException(self.identifier, "mini_bandwidth", \
                f"PSACompilationFOOOF input of wrong type or empty. Expected: str of float") 
        try:
        ...
        if isinstance(report_constants,str) and report_constants == '':
            raise NodeInputException(self.identifier, "report_constants", \
                "PSACompilationFOOOF report_constants parameter must be set.")
        elif isinstance(report_constants,str):
            report_constants = eval(report_constants)
        if isinstance(report_constants,dict) == False:
            raise NodeInputException(self.identifier, "report_constants",\
                "PSACompilationFOOOF report_constants expected type is dict and received type is " + str(type(report_constants)))   
        ...
        if (not isinstance(filename,str)) or (isinstance(filename,str) and len(filename)==0):
            raise NodeInputException(self.identifier, "filename", \
                f"PSACompilationFOOOF input of wrong type. Expected: <class str> received: {type(filename)}") 
        ...
        for channel in channels_list:
            ...
            if int(dist_total)==1:
                my_label = 'total'
                PSD_rhythmic_avg_total, PSD_arhythmic_avg_total, PSD_rhythmic_avg_per_stage, PSD_arhythmic_avg_per_stage = \
                    self.compute_fft_win_div(psd_data, psd_stage, my_label, first_freq, last_freq, freq_bins,
                                            self.PSD_rhythmic_act_param, self.PSD_arhythmic_act_param)

                # Compute and store activity for rhythmic
                self.PSD_rhythmic_act_param[f"total_act"] = \
                    [np.nansum(PSD_rhythmic_avg_total[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                # Compute and store activity for arhythmic
                self.PSD_arhythmic_act_param[f"total_act"] = \
                    [np.nansum(PSD_arhythmic_avg_total[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]

                for i, stage_label in enumerate(self.stage_stats_labels):
                    if (self.PSD_rhythmic_act_param[f'total_{stage_label}_fft_win_count']>0):
                        # The fft normalisation is made to integrate (sum) through frequency bins
                        self.PSD_rhythmic_act_param[f"total_{stage_label}_act"] = \
                            [np.nansum(PSD_rhythmic_avg_per_stage[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_rhythmic_act_param[f"total_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))
                        
                    if (self.PSD_arhythmic_act_param[f'total_{stage_label}_fft_win_count']>0):
                        # The fft normalisation is made to integrate (sum) through frequency bins
                        self.PSD_arhythmic_act_param[f"total_{stage_label}_act"] = \
                            [np.nansum(PSD_arhythmic_avg_per_stage[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_arhythmic_act_param[f"total_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices)) 
            ...
            if int(dist_hour)==1: 
                ...
                for cur_div in range(self.N_HOURS):
                    ...
                    PSD_rhythmic_avg_hour, PSD_arhythmic_avg_hour, PSD_rhythmic_stage_avg_hour, PSD_arhythmic_stage_avg_hour = \
                        self.compute_fft_win_div(psd_data_div, psd_stage_div, f'{my_label1}{cur_div+1}', first_freq, last_freq, freq_bins,
                                                self.PSD_rhythmic_act_param, self.PSD_arhythmic_act_param)

                    # Rhythmic activity
                    if self.PSD_rhythmic_act_param[f'{my_label1}{cur_div+1}_fft_win_count']>0:
                        self.PSD_rhythmic_act_param[f"{my_label1}{cur_div+1}_act"] = \
                            [np.nansum(PSD_rhythmic_avg_hour[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_rhythmic_act_param[f"{my_label1}{cur_div+1}_act"] = np.array([np.NaN]*len(miniband_indices))
                        
                    # Arhythmic activity
                    if self.PSD_arhythmic_act_param[f'{my_label1}{cur_div+1}_fft_win_count']>0:
                        self.PSD_arhythmic_act_param[f"{my_label1}{cur_div+1}_act"] = \
                            [np.nansum(PSD_arhythmic_avg_hour[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_arhythmic_act_param[f"{my_label1}{cur_div+1}_act"] = np.array([np.NaN]*len(miniband_indices))
                    
                    for i, stage_label in enumerate(self.stage_stats_labels):
                        if (self.PSD_rhythmic_act_param[f'{my_label1}{cur_div+1}_{stage_label}_fft_win_count']>0):
                            # The fft normalisation is made to integrate (sum) through frequency bins
                            self.PSD_rhythmic_act_param[f"{my_label1}{cur_div+1}_{stage_label}_act"] = \
                                [np.nansum(PSD_rhythmic_stage_avg_hour[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                        else:
                            self.PSD_rhythmic_act_param[f"{my_label1}{cur_div+1}_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))
                            
                        if (self.PSD_arhythmic_act_param[f'{my_label1}{cur_div+1}_{stage_label}_fft_win_count']>0):
                            # The fft normalisation is made to integrate (sum) through frequency bins
                            self.PSD_arhythmic_act_param[f"{my_label1}{cur_div+1}_{stage_label}_act"] = \
                                [np.nansum(PSD_arhythmic_stage_avg_hour[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                        else:
                            self.PSD_arhythmic_act_param[f"{my_label1}{cur_div+1}_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))  
                    
                    # Calculate FFT window counts and PSD averages for stage_h data
                    PSD_rhythmic_avg_stage_hour, PSD_arhythmic_avg_stage_hour, PSD_rhythmic_stage_avg_stage_hour, PSD_arhythmic_stage_avg_stage_hour = \
                        self.compute_stage_hour_statistics(psd_data, stage_window_data, stage_window_counts, stage_valid_counts, win_len, cur_div,
                                                          first_freq, last_freq, freq_bins, self.PSD_rhythmic_act_param, self.PSD_arhythmic_act_param)
                    
                    # Calculate spectral activity for stage_h columns using the returned PSD averages - rhythmic
                    if self.PSD_rhythmic_act_param[f'{my_label2}{cur_div+1}_fft_win_count']>0:
                        self.PSD_rhythmic_act_param[f"{my_label2}{cur_div+1}_act"] = \
                            [np.nansum(PSD_rhythmic_avg_stage_hour[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_rhythmic_act_param[f"{my_label2}{cur_div+1}_act"] = np.array([np.NaN]*len(miniband_indices))
                        
                    # Calculate spectral activity for stage_h columns using the returned PSD averages - arhythmic
                    if self.PSD_arhythmic_act_param[f'{my_label2}{cur_div+1}_fft_win_count']>0:
                        self.PSD_arhythmic_act_param[f"{my_label2}{cur_div+1}_act"] = \
                            [np.nansum(PSD_arhythmic_avg_stage_hour[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_arhythmic_act_param[f"{my_label2}{cur_div+1}_act"] = np.array([np.NaN]*len(miniband_indices))
                        
                    for i, stage_label in enumerate(self.stage_stats_labels):
                        if (self.PSD_rhythmic_act_param[f'{my_label2}{cur_div+1}_{stage_label}_fft_win_count']>0):
                            # The fft normalisation is made to integrate (sum) through frequency bins
                            self.PSD_rhythmic_act_param[f"{my_label2}{cur_div+1}_{stage_label}_act"] = \
                                [np.nansum(PSD_rhythmic_stage_avg_stage_hour[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                        else:
                            self.PSD_rhythmic_act_param[f"{my_label2}{cur_div+1}_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))
                            
                        if (self.PSD_arhythmic_act_param[f'{my_label2}{cur_div+1}_{stage_label}_fft_win_count']>0):
                            # The fft normalisation is made to integrate (sum) through frequency bins
                            self.PSD_arhythmic_act_param[f"{my_label2}{cur_div+1}_{stage_label}_act"] = \
                                [np.nansum(PSD_arhythmic_stage_avg_stage_hour[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                        else:
                            self.PSD_arhythmic_act_param[f"{my_label2}{cur_div+1}_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))
            ...
            if int(dist_cycle)==1:
                ...
                for cur_div in range(self.N_CYCLES):
                    ...
                    self.PSD_rhythmic_act_param[f'{my_label}{cur_div+1}_length_min'] = (end_cycle-start_cycle)/60
                    self.PSD_arhythmic_act_param[f'{my_label}{cur_div+1}_length_min'] = (end_cycle-start_cycle)/60
                    ...
                    PSD_rhythmic_avg_cycle, PSD_arhythmic_avg_cycle, PSD_rhythmic_stage_avg_cycle, PSD_arhythmic_stage_avg_cycle = \
                        self.compute_fft_win_div(psd_data_div, psd_stage_div, f'{my_label}{cur_div+1}', first_freq, last_freq, freq_bins,
                                                self.PSD_rhythmic_act_param, self.PSD_arhythmic_act_param)

                    # Rhythmic activity
                    if self.PSD_rhythmic_act_param[f'cyc{cur_div+1}_fft_win_count']>0:
                        self.PSD_rhythmic_act_param[f"cyc{cur_div+1}_act"] = \
                            [np.nansum(PSD_rhythmic_avg_cycle[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_rhythmic_act_param[f"cyc{cur_div+1}_act"] = np.array([np.NaN]*len(miniband_indices))
                        
                    # Arhythmic activity
                    if self.PSD_arhythmic_act_param[f'cyc{cur_div+1}_fft_win_count']>0:
                        self.PSD_arhythmic_act_param[f"cyc{cur_div+1}_act"] = \
                            [np.nansum(PSD_arhythmic_avg_cycle[miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                    else:
                        self.PSD_arhythmic_act_param[f"cyc{cur_div+1}_act"] = np.array([np.NaN]*len(miniband_indices))
                        
                    for i, stage_label in enumerate(self.stage_stats_labels):
                        if (self.PSD_rhythmic_act_param[f'cyc{cur_div+1}_{stage_label}_fft_win_count']>0):
                            # The fft normalisation is made to integrate (sum) through frequency bins
                            self.PSD_rhythmic_act_param[f"cyc{cur_div+1}_{stage_label}_act"] = \
                                [np.nansum(PSD_rhythmic_stage_avg_cycle[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                        else:
                            self.PSD_rhythmic_act_param[f"cyc{cur_div+1}_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))
                            
                        if (self.PSD_arhythmic_act_param[f'cyc{cur_div+1}_{stage_label}_fft_win_count']>0):
                            # The fft normalisation is made to integrate (sum) through frequency bins
                            self.PSD_arhythmic_act_param[f"cyc{cur_div+1}_{stage_label}_act"] = \
                                [np.nansum(PSD_arhythmic_stage_avg_cycle[i][miniband_index[0]:miniband_index[1]+1]) for miniband_index in miniband_indices]
                        else:
                            self.PSD_arhythmic_act_param[f"cyc{cur_div+1}_{stage_label}_act"] = np.array([np.NaN]*len(miniband_indices))       
            ...
            # Rhythmic report
            mini_band_dict_rhythmic = PSD_freq_params | self.PSD_rhythmic_act_param
            mini_band_df_rhythmic = pd.DataFrame.from_dict(mini_band_dict_rhythmic)
            cur_chan_report_df_rhythmic = pd.concat([mini_band_hdr_df, mini_band_df_rhythmic], axis=1)

            if len(report_df_rhythmic):
                report_df_rhythmic = pd.concat([report_df_rhythmic, cur_chan_report_df_rhythmic], axis=0) # Add the rows for the current channel to the total report
            else:
                report_df_rhythmic = cur_chan_report_df_rhythmic
            
            # Arhythmic report
            mini_band_dict_arhythmic = PSD_freq_params | self.PSD_arhythmic_act_param
            mini_band_df_arhythmic = pd.DataFrame.from_dict(mini_band_dict_arhythmic)
            cur_chan_report_df_arhythmic = pd.concat([mini_band_hdr_df, mini_band_df_arhythmic], axis=1)

            if len(report_df_arhythmic):
                report_df_arhythmic = pd.concat([report_df_arhythmic, cur_chan_report_df_arhythmic], axis=0) # Add the rows for the current channel to the total report
            else:
                report_df_arhythmic = cur_chan_report_df_arhythmic
        ...
        report_df_rhythmic = report_df_rhythmic[out_columns] # To ensure the column order
        report_df_arhythmic = report_df_arhythmic[out_columns] # To ensure the column order
        ...
        try: 
            report_df_rhythmic.to_csv(path_or_buf=filename_rhythmic, sep='\t', \
                index=False, mode='a', header=write_header_rhythmic, encoding="utf_8")
        except:
            raise NodeRuntimeException(self.identifier, "PSA", f"ERROR : Snooz can not write in the file {filename_rhythmic}. Check if the drive is accessible and ensure the file is not already open.")
        
        # Write arhythmic report
        write_header_arhythmic = not os.path.exists(filename_arhythmic)
        try: 
            report_df_arhythmic.to_csv(path_or_buf=filename_arhythmic, sep='\t', \
                index=False, mode='a', header=write_header_arhythmic, encoding="utf_8")
        except:
            raise NodeRuntimeException(self.identifier, "PSA", f"ERROR : Snooz can not write in the file {filename_arhythmic}. Check if the drive is accessible and ensure the file is not already open.")
        ...
        if write_header_rhythmic:
            ...
            if not os.path.exists(doc_filepath_rhythmic):
                try:
                    ...
                    self._log_manager.log(self.identifier, f"The file {doc_filepath_rhythmic} has been created.")
                except:
                    raise NodeRuntimeException(self.identifier, "PSA", f"ERROR : Snooz can not write in the file {doc_filepath_rhythmic}. Check if the drive is accessible and ensure the file is not already open.")
        ...
        if write_header_arhythmic:
            ...
            if not os.path.exists(doc_filepath_arhythmic):
                try:
                    ...
                    # Log message for the Logs tab
                    self._log_manager.log(self.identifier, f"The file {doc_filepath_arhythmic} has been created.")
                except:
                    raise NodeRuntimeException(self.identifier, "PSA", f"ERROR : Snooz can not write in the file {doc_filepath_arhythmic}. Check if the drive is accessible and ensure the file is not already open.")    
        return {
        }
    ...
    def get_artefact_info(self, artefact_group, artefact_name, events):
        ...
        if len(art_group_lst)==len(art_name_lst):
            ...
            raise NodeInputException(self.identifier, "artefact_group", \
                f"PSACompilationFOOOF input of wrong size. If both group and name are defined they need to be the same length, group length:\
    ...
    def compute_fft_win_div(self, psd_data, psd_stage, label_stat_div, first_freq, last_freq, freq_bins, 
                            rhythmic_act_param, arhythmic_act_param):
        """
            Compute the total and the valid number of fft windows for each sleep stage
            defined by sleep_stage_to_stats. Populate both rhythmic and arhythmic parameter dictionaries.
            ...
            Inputs:
                ...
                label_stat_div : str
                    String to label the stats per division (.i.e. total, clock_h1, cyc1...)
                rhythmic_act_param : dict
                    Dictionary to store rhythmic activity parameters
                arhythmic_act_param : dict
                    Dictionary to store arhythmic activity parameters

            return : 
                PSD_rhythmic_avg_total : array
                    the total rhythmic average PSD (len of the frequency bins)
                PSD_arhythmic_avg_total : array
                    the total arhythmic average PSD (len of the frequency bins)
                PSD_rhythmic_avg_per_stage : list
                    the average rhythmic PSD per sleep stage (len of the frequency bins)
                PSD_arhythmic_avg_per_stage : list
                    the average arhythmic PSD per sleep stage (len of the frequency bins)

        """
        PSD_rhythmic_avg_per_stage = []
        PSD_arhythmic_avg_per_stage = []
        ...
        for i_list, cur_stage_list in enumerate(self.sleep_stage_to_stats):
            ...
            # Populate both rhythmic and arhythmic parameter dicts with window counts
            ...
            rhythmic_act_param[n_fft_win_label] = div_n_fft_win
            rhythmic_act_param[n_fft_valid_label] = div_n_fft_win_valid
            arhythmic_act_param[n_fft_win_label] = div_n_fft_win
        ...
        # Populate both dictionaries with total window counts
        rhythmic_act_param[f'{label_stat_div}_fft_win_count'] = tot_n_fft_win
        rhythmic_act_param[f'{label_stat_div}_fft_win_valid_count'] = tot_n_fft_win_valid
        arhythmic_act_param[f'{label_stat_div}_fft_win_count'] = tot_n_fft_win
        arhythmic_act_param[f'{label_stat_div}_fft_win_valid_count'] = tot_n_fft_win_valid
    ...
    def compute_stage_hour_statistics(self, psd_data, stage_window_data, stage_window_counts, stage_valid_counts, win_len, hour_label,
                                      first_freq, last_freq, freq_bins, rhythmic_act_param, arhythmic_act_param):
        """
        Calculate FFT window statistics and PSD averages for a specific hour based on 
        pre-collected stage data. Populate both rhythmic and arhythmic parameter dictionaries.
        ...
        --------
        PSD_rhythmic_avg_total : array
            The total rhythmic average PSD across selected windows for this hour
        PSD_arhythmic_avg_total : array
            The total arhythmic average PSD across selected windows for this hour
        PSD_rhythmic_avg_per_stage : list
            The average rhythmic PSD per sleep stage for selected windows for this hour
        PSD_arhythmic_avg_per_stage : list
            The average arhythmic PSD per sleep stage for selected windows for this hour
        """
        ...
        for i_list, cur_stage_list in enumerate(self.sleep_stage_to_stats):
            ...
            if cur_stage_win_count // ((hour_label + 1) * expected_windows_per_hour) > 0:
                rhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_count'] = expected_windows_per_hour
                arhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_count'] = expected_windows_per_hour
                ...
                count = cur_stage_win_count - (hour_label * expected_windows_per_hour)
                rhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_count'] = count
                arhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_count'] = count
                start_idx = int(hour_label * expected_windows_per_hour)
                psd_data_cur_stage_cur_hour = psd_data_cur_stage[start_idx:]
            else:
                rhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_count'] = 0
                arhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_count'] = 0
            ...
            if valid_cur_stage_win_count // ((hour_label + 1) * expected_windows_per_hour) > 0:
                rhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_valid_count'] = expected_windows_per_hour
                arhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_valid_count'] = expected_windows_per_hour
            elif valid_cur_stage_win_count > hour_label * expected_windows_per_hour:
                count = valid_cur_stage_win_count - (hour_label * expected_windows_per_hour)
                rhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_valid_count'] = count
                arhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_valid_count'] = count
            else:
                rhythmic_act_param[f'stage_h{hour_label+1}_{stage_label}_fft_win_valid_count'] = 0
            ...
            if len(psd_data_cur_stage_cur_hour) > 0 and not np.all(np.isnan(psd_data_cur_stage_cur_hour)):
                ...
                empty_array = np.array([np.NaN] * len(psd_data_cur_stage[0]) if len(psd_data_cur_stage) > 0 else [])
                PSD_rhythmic_avg_per_stage.append(empty_array)
                PSD_arhythmic_avg_per_stage.append(empty_array)
        ...
        if total_windows // ((hour_label + 1) * expected_windows_per_hour) > 0:
            rhythmic_act_param[f'stage_h{hour_label+1}_fft_win_count'] = expected_windows_per_hour
            arhythmic_act_param[f'stage_h{hour_label+1}_fft_win_count'] = expected_windows_per_hour
            ...
            count = total_windows - (hour_label * expected_windows_per_hour)
            rhythmic_act_param[f'stage_h{hour_label+1}_fft_win_count'] = count
            arhythmic_act_param[f'stage_h{hour_label+1}_fft_win_count'] = count
            psd_data_tot = psd_data[int(hour_label * expected_windows_per_hour):]
        else:
            rhythmic_act_param[f'stage_h{hour_label+1}_fft_win_count'] = 0
            arhythmic_act_param[f'stage_h{hour_label+1}_fft_win_count'] = 0
        
        # Similar logic for valid counts
        if total_valid_windows // ((hour_label + 1) * expected_windows_per_hour) > 0:
            rhythmic_act_param[f'stage_h{hour_label+1}_fft_win_valid_count'] = expected_windows_per_hour
            arhythmic_act_param[f'stage_h{hour_label+1}_fft_win_valid_count'] = expected_windows_per_hour
        elif total_valid_windows > hour_label * expected_windows_per_hour:
            count = total_valid_windows - (hour_label * expected_windows_per_hour)
            rhythmic_act_param[f'stage_h{hour_label+1}_fft_win_valid_count'] = count
            arhythmic_act_param[f'stage_h{hour_label+1}_fft_win_valid_count'] = count
        else:
            rhythmic_act_param[f'stage_h{hour_label+1}_fft_win_valid_count'] = 0
        ...
        if len(psd_data_tot) > 0 and not np.all(np.isnan(psd_data_tot)):
            ...
            PSD_arhythmic_avg_total = np.array([np.NaN] * len(psd_data[0]) if len(psd_data) > 0 else [])
        
        return PSD_rhythmic_avg_total, PSD_arhythmic_avg_total, PSD_rhythmic_avg_per_stage, PSD_arhythmic_avg_per_stage

    def FOOOF(self, psd_data, first_freq, last_freq, freq_bins):
        """
        Apply FOOOF to the average PSD data across the recording for each period and stage. 
        The FOOOF parameters are set to default values that can be modified in the future if needed.
