...
See the file LICENCE for full license details.
    ...
    GroupDefinition
    The step to define the group of each PSG recording for PSA Images.
"""
...
from CEAMSTools.PSAImages.InputFiles.InputFiles import InputFiles
from CEAMSTools.PSAImages.GroupDefinition.Ui_GroupDefinition import Ui_GroupDefinition
from CEAMSTools.PSAImages.GroupDefinition.FileProxyModel import FileProxyModel


class GroupDefinition(BaseStepView, Ui_GroupDefinition, QtWidgets.QWidget):
    """
        GroupDefinition for PSA Images
        The step to define the group of each PSA recording.
    """
    def __init__(self, **kwargs):
        ...
        # Store filenames from InputFiles step
        self.filenames = []
        ...
        self._psa_images_node = "bc3ee945-0abf-46b7-b110-db5e617ea70a"  # PSA Images node ID
        # Subscribe to the proper topics to send/get data from the node
        self._file_group_topic = f'{self._psa_images_node}.file_group'
    ...
    def on_topic_update(self, topic, message, sender):
        ...
        if topic == self._context_manager.topic:
            if message == InputFiles.context_InputFiles:
                # Access to the filenames from InputFiles context
                filenames_data = self._context_manager[InputFiles.context_InputFiles]
                if isinstance(filenames_data, dict) and 'filenames' in filenames_data:
                    self.filenames = filenames_data['filenames']
                elif isinstance(filenames_data, list):
                    self.filenames = filenames_data
                else:
                    self.filenames = []
                # Create the model for the checkable table, based on the filenames
    ...
    def on_topic_response(self, topic, message, sender):
        if topic == self._file_group_topic:
            ...
            if isinstance(message, dict):
                ...
                self.group_dict = {}
            # Create or update the model for the table view, based on self.filenames and self.group_dict
            self.files_table_model = self.update_table_model()


    def on_apply_settings(self):
        # Send the dictionary as an input to the PSAPicsGenerator module
    ...
    def update_table_model(self):
        # Create the model for the checkable table, based on self.filenames
        for filename in self.filenames:
            # Add in the table view self.tableView_group the files included in self.filenames
