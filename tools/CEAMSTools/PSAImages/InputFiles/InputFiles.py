...
from CEAMSTools.PSAImages.InputFiles.Ui_InputFiles import Ui_InputFiles
...
class InputFiles(BaseStepView, Ui_InputFiles, QtWidgets.QWidget):
    ...
    def __init__(self, **kwargs):
        ...
        self._PSA_pics_identifier = "bc3ee945-0abf-46b7-b110-db5e617ea70a"

        # To use the SettingsView of a plugin and interract with its fonctions
        module = self.process_manager.get_node_by_id(self._PSA_pics_identifier)
        if module is None:
            print(f'ERROR module_id isn\'t found in the process:{self._PSA_pics_identifier}')
        else:
            # To extract the SettingsView and add it to our Layout in the preset
            self.my_SettingsView = module.create_settings_view()
            self.verticalLayout_2.addWidget(self.my_SettingsView)   
