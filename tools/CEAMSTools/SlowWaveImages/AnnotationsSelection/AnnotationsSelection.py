...
"""
    ...
    Step in the SlowWaveImages interface to select the annotations in order to generate the images.
    Copied from NonValidEventStep in the SpectralAnalysis tool.
    The only difference is the way the dictionary of the states are managed.
    They are not different dict to provide different dict plugins, it's directly 
    a input of the SlowWavePicsGenerator plugin.
...
class AnnotationsSelection( BaseStepView,  Ui_NonValidEventStep, QtWidgets.QWidget):
    ...
    def __init__(self, **kwargs):
        ...
        self._node_id_ResetSignalArtefact_0 = None # to activate if the signal during artifact are reset
        ...
        self._sw_events_node = f'{self._sw_wave_pics_node}.sw_events_def'
        self._pub_sub_manager.subscribe(self, self._sw_events_node)

        # Dict to keep track of selected events
        self.group_name_dict = {} # the key is the filename and the value is a dict with group and name of sw events

        self.reader_settings_view = None
    ...
    def load_settings(self):
        ...
        self._pub_sub_manager.publish(self, self._sw_events_node, 'ping')
        # group_name_dict is updated from the pipeline.json
        ...
        if self._node_id_ResetSignalArtefact_0 is not None:
            if topic == self._node_id_ResetSignalArtefact_0+".get_activation_state":
                ...
                if message==ActivationState.BYPASS:
                    self.reset_excl_event_checkBox.setChecked(False)
        if topic == self._sw_events_node:
            if isinstance(message,dict):
                self.group_name_dict = message
            elif isinstance(message,str) and not message=="":
                self.group_name_dict = ast.literal_eval(message)
            else:
    ...
    def on_apply_settings(self):
        ...
        self._pub_sub_manager.publish(self, self._sw_events_node, str(self.group_name_dict))
    ...
    def _set_event_lst_to_view(self):
        if len(self.group_name_dict)>0:
            # For each opened file
            for file_key in self.group_name_dict:
                # Extract which event groups and names are selected
                group_lst = self.group_name_dict[file_key]['group']
                group_lst = group_lst.split(',')
                name_list = self.group_name_dict[file_key]['name']
    ...
    def _store_event_dict(self):       
        ...
        for i_file in range(n_files):
            ...
            #     group_name_dict[filename] = {'group':'None', 'name':'None'}
            # If the file is already in the model
            if isinstance(file_checkable_index, QtCore.QModelIndex):
                ...
                if len(group_list)>0:
                    ...
                    group_name_dict[filename] = {'group':",".join(group_list), 'name':",".join(name_list)}
                else:
                    group_name_dict[filename] = {'group':'None', 'name':'None'}
            # else:
            #     raise NodeRuntimeException(self.identifier, "_store_event_dict", \
            #         f"AnnotationsSelection : the selected index is not valid")
    ...
    def __del__(self):
        if self._pub_sub_manager is not None:
            self._pub_sub_manager.unsubscribe(self, self._sw_events_node)
