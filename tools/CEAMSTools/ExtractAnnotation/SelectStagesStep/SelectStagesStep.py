...
from CEAMSTools.ExtractAnnotation.InputFiles.InputFiles import InputFiles
...
class SelectStagesStep( BaseStepView,  Ui_SelectStagesStep, QtWidgets.QWidget):
    ...
    node_id_Dictionary_group = "73f19acb-36f0-45b6-a1ab-b07a09f8ca58" # select the list of group for the current filename
    node_id_Dictionary_name = "dc4ff07e-d8bf-472b-a308-8706ddbacf76" # select the list of name for the current filename
    ...
    def init_models(self):
        ...
        self.reader_settings_view = self._context_manager[InputFiles.context_files_view]
        # Extract the model (where the file information is stored) 
        #   To be aware of any change from the InputFiles Step
        self.files_model = self.reader_settings_view.files_stages_model
    ...
    def on_topic_update(self, topic, message, sender):
        ...
        if topic==self._context_manager.topic:
            if message==InputFiles.context_files_view: # key of the context dict
    ...
    def update_checkable_model(self, checkable_model_outdated):
        if checkable_model_outdated == None:
            # Create checkable item based on self.reader_settings_view.files_stages_model
